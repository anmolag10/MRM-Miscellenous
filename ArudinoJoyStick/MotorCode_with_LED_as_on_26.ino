...
x=analogRead(xaxis);
y=analogRead(yaxis);
//calibration for x and y
//x=x+K; // constvalue K is x calibration factor: K=512-x
//y=y+H //constvalue H  is y calibration factor: H=512-y





if((y==512) && (x=512)) // to check if it is origin
{ 
  rval=0;
  lval=0;
}
